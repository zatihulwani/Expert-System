...
def k():
    if 'loggedin' in session:
        return render_template('cfmknn.html',username=session['username'])
    return redirect(url_for('login'))
...
def riwayatpasien():
    if 'loggedin' in session:
        ...
        rm = cur.fetchall()
        jumlahRiwayatPasien = len(rm)
        cur.close()
        return render_template('riwayatpasien.html',jumlahRiwayatPasien=jumlahRiwayatPasien, data=rm,username=session['username'])
    return redirect(url_for('login'))
...
def riwayatpasien2():
    if 'loggedin' in session:
        ...
        cur.execute("SELECT * FROM gejala")
        gejala = cur.fetchall()
        cur.execute("SELECT * FROM exx")
        exx = cur.fetchall()
        cur.close()
        return render_template('riwayatpasien2.html', gejala=gejala, data=exx,username=session['username'])
    return redirect(url_for('login'))
...
def sakti():
    ...
    newItem = {'GP01': 0 ,'GP02': 0,'GP03': 0,'GP04': 0,'GP05': 0,'GP06': 0,'GP07': 0,'GP08': 0,'GP09': 0,'GP10': 0,'GP11': 0,'GP12': 0,'GP13': 0,'GP14': 0,'GP15': 0,'GP16': 0,'GP17': 0}
    ...
    # data testing
    newItem = {'GP01': GP01 ,'GP02': GP02,'GP03': GP03, 'GP04': GP04,'GP05': GP05,'GP06': GP06,'GP07': GP07,'GP08': GP08,'GP09': GP09,'GP10': GP10,'GP11': GP11,'GP12': GP12,'GP13': GP13,'GP14': GP14,'GP15': GP15,'GP16': GP16,'GP17':GP17}
